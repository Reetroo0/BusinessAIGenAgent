...
system_prompt = (
    ...
    "При использовании инструмента find_matching_vacancies: "
    "1. Передавай параметры user_skills и experience_level из профиля пользователя "
    "2. ВАЖНО: Не форматируй и не изменяй вывод инструмента, он уже содержит всю нужную информацию включая ссылки "
    "3. Возвращай результат поиска вакансий как есть, без дополнительного форматирования "
    "4. Не удаляй ссылки (🔗) из результатов поиска вакансий "
    "5. Не преобразуй текст в Markdown формат "
    "user_skills и experience_level из профиля пользователя. Используй список known_technologies "
    "как user_skills и поле experience как experience_level. "
...
def run_agent(question: str, user_profile: Optional[Dict] = None, headers: Optional[Dict] = None) -> str:
    ...
    try:
        resp = agent.invoke({"messages": messages}, config=config, recursion_limit=10)
...
def process_career_query(user_id: str, query: str, session_data: Optional[Dict] = None, 
    ...
    if user_data:
        # Убедимся, что у нас есть базовые поля для работы с вакансиями
        profile = session_data.setdefault("profile", {})
        profile.update(user_data)
        
        # Если known_technologies не указаны, попробуем извлечь их из текста
        if 'known_technologies' not in profile:
            from tools import extract_skills_from_text
            profile['known_technologies'] = extract_skills_from_text(query)
        
        # Если experience не указан, попробуем извлечь его из текста
        if 'experience' not in profile:
            from tools import extract_experience
            profile['experience'] = extract_experience(query)
